...
# Original script get from https://medium.com/@z.baratz/setting-up-keepassxc-on-linux-with-cloud-synchronization-85ccce837365
...
function passwords_import() {
  rclone copy "$LOCAL_PATH" "$DRIVE_NAME:$REMOTE_PATH $LOCAL_LOCATION"
}
...
function sync_passwords ()
{
	...
	# Storing the values so they can be used for printing and then conversion
        local human_readable_local_mtime
